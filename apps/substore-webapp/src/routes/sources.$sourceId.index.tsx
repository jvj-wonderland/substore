...
function ViewSourcePage() {
  ...
  const fennelQuery = useQuery({
    queryKey: ["sources", sourceId, "fennel"],
    queryFn: () =>
      Effect.runPromise(
        API.transformToFennel({
          content: sourceQuery.data?.content ?? "",
        }).pipe(Effect.provide(API.clientLayer))
      ),
                ...
                <ResizablePanelGroup orientation="vertical" className="h-full">
                  ...
                  <ResizablePanel defaultSize={50} minSize={20}>
                    <div className="flex h-full min-h-0 flex-col overflow-hidden">
                      ...
                      <div className="flex-1 overflow-auto p-4">
                        {fennelQuery.data?.fennel ? (
                          <CodeBlock
                            code={fennelQuery.data.fennel}
                            lang="fennel"
                          />
                        ) : (
