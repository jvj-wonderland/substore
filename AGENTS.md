...
- **Component Modularity**: Large components (e.g., source/sink editors) MUST be broken down into domain-specific sub-components (e.g., `SourceEditorHeader`, `SourcePreviewPanel`) when they exceed 300 lines or become logically complex.
- **Route UI Abstraction**: Route files (`src/routes/*.tsx`) SHOULD NOT contain complex UI or layout logic. Abstract the entire page rendering into a reusable component (e.g., `SinkEditorPage`) to separate data-fetching/mutation logic from the presentation layer.
- **Naming Consistency**: Use descriptive, domain-prefixed names for extracted sub-components (e.g., `SourceEditorLocalFields` instead of `LocalFields`) to ensure clarity and avoid namespace collisions within the `components/` directory.
